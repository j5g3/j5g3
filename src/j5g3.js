...
/**
 * @license
 * j5p4 v0.5 - Javascript Physics Engine 
 * http://
 *
...
/*jshint white:false smarttabs:true es5:true */
...
/**
 * @license
 * j5g3 v0.5 - Javascript Game Engine 
 * http://j5g3.com
 *
 * Copyright 2010-2012, Giancarlo F Bellido
 *
...
var 
	...
	g3 = 
	/** Returns a new j5.g3.GameEngine */
	j5.g3 = function j5g3(p) { return new j5.g3.GameEngine(p); },
...
j5.g3.Collision = {
	...
	var
		l1 = this.x, 
		t1 = this.y, 
		r1 = this.M.x(this.width, this.height),
		b1 = this.M.y(this.width, this.height),
		l2 = obj.x,
		t2 = obj.y,
		r2 = obj.M.x(obj.width, obj.height),
		b2 = obj.M.y(obj.width, obj.height),
		tmp
	;
		/*if (l1 > r1) { tmp = r1; r1 = l1; l1 = tmp; }
		if (t1 > b1) { tmp = b1; b1 = t1; t1 = tmp; }
		if (l2 > r2) { tmp = r2; r2 = l2; l2 = tmp; }
		if (t2 > b2) { tmp = b2; b2 = t2; t2 = tmp; }
		*/
...
j5.g3.DisplayObject = Class.extend(/** @scope j5.g3.DisplayObject.prototype */ {
	...
	set x(val) { return this.M.e = val; },
	get x() { return this.M.e; },

	/** @type {number} Y position */
	set y(val) { return this.M.f = val; },
	...
	;
		...
		if (me.fillStyle!==null) context.fillStyle = me.fillStyle;
		if (me.strokeStyle!==null) context.strokeStyle = me.strokeStyle;
		if (me.font) context.font = me.font;
		if (me.blending) context.globalCompositeOperation=me.blending;

		if (me.lineWidth!==null) context.lineWidth = me.lineWidth;
		if (me.lineCap!==null) context.lineCap = me.lineCap;
		if (me.lineJoin!==null) context.lineJoin = me.lineJoin;
		if (me.miterLimit!==null) context.miterLimit = me.miterLimit;
...
j5.g3.Stage = Clip.extend({
	...
	{
		//j5.clipping = clip = x!==false ? { x: x, y: y, w: w, h: h } : null;
	}
...
/** @scope j5.g3.Shape.prototype */ {
	...
	/**
	 * Red value for fillStyle attribute. Make sure to set all three for 
	 * colors to work. 0 to 255.
	 */
	red: undefined, 

	/**
	 * Green value for fillStyle attribute. Make sure to set all three for 
	 * colors to work. 0 to 255.
	 */
	green: undefined, 
	
	/**
	 * Blue value for fillStyle attribute. Make sure to set all three for 
	 * colors to work. 0 to 255.
	 */
	blue: undefined,
	...
	begin: function(context)
	{
		...
		if (me.red!==me._red || me.blue!==me._blue || me.green!==me._green)
		{
			...
			me[me.colorProperty] = 'rgb(' + Math.floor(me.red) + ',' + 
				Math.floor(me.green) + ',' + 
				Math.floor(me.blue) + ')'
			;
			me._red = me._red;
...
j5.g3.Polygon = Shape.extend(/**@scope j5.g3.Polygon.prototype */{
	...
	{
	var
		i=0, a, points = this.points, l=points.length, dx, dy,
...
j5.g3.Emitter = Clip.extend(/**@scope j5.g3.Emitter.prototype */ {
	...
	;
		...
		frames.push([ j5.g3.action(function() { 
			clip.remove();
		}) ]);
...
/** @scope j5.g3.Action.prototype */ {
	...
	{
		this.draw = j5.getType(p)=== 'function' ? p: p.code;
	},
...
j5.g3.GameEngine = Class.extend({
	 ...
	 * Starts Engine
	 */
	init: function j5g3GameEngine(config) 
	{
	var
		me = this
	;
		...
		me.set_stage(new Stage({ canvas: config.canvas }))
			.startFn(j5, g3)
		;
	},
...
/**
 * @license
 * j5fx v0.5 - Javascript Effects Module 
 * http://
 *
 * Copyright 2010-2012, Giancarlo F Bellido
 *
...
j5.fx = { 
	...
	Text: 
	{
		...
		var
			...
			color  = this.strokeStyle || this.fillStyle,
			lw = this.lineWidth
		;
		...
		Mirror: function(context)
		{
			...
			if (!this.gradient)
			{
				...
				this.gradient.addColorStop(0, this.fillStyle);
